...
Discord hat leider ein paar Einschränkungen.
Hierzu gehören Channel Limits, welche auf 500 pro Discord Server limitiert sind. \autocite{roonie_latest_2022}
Diese können auf maximal 250 Kategorien aufgeteilt werden mit je maximal 50 Subchannels.
Die Einschränkungen werden auf dem Discord Support Forum diskutiert, jedoch haben sich die Discord Arbeiter stehts an der Position festgehalten, die Limite sei hoch genug und es gäbe keine Sinnvollen nutzen an einer höheren Limite. \autocite{noauthor_increase_nodate}
Dies könnte zukünftig zu Problemen führen, da es zur Zeit 533 Module gibt, welche besucht werden können.
Dies beinhaltet ebenfalls ISA und diverse erweiterungs Module.
Erfahrungen aus dem letzten Semester haben jedoch gezeigt, dass nur ein Bruchteil der Modulchannels genutzt wird, weshalb die Limite noch länger nicht erreicht werden sollte.
Die Anzahl Rollen pro Discord Server ist ebenfalls eingeschränkt.
Dies führt dazu, dass Studenten nicht über die Rollen die Modul-Channels freigegeben bekommen können, da dies dann den Rahmen sprengen würde. \autocite{noauthor_increase_nodate-1}
Dies ist der Grund, dass die Studenten einzeln pro Channel freigegeben werden, anstatt dass diese eine neue Rolle bekommen, wie in der bisherigen Umsetzung.
...
\end{table}
\todo{Risikobereitschaftslinie irgendwie einzeichnen.}
...
Das genaue Format wird in der Anleitung beschrieben.\todo{Kapitel Verweis einfügen}
...
Diese müssen gut überlegt sein, da man nur für spezifizierte Events Callbacks im Code bekommt.
\newline
...
\todo{Bild Zuweisung von Bot zum Server}
...
\todo{Yannis}
\todo{add connection string | remove password}
\begin{lstlisting}
...
\todo{Einfügen des Bildes der Discord Developer Einstellungen}
\newline
...
\todo{Könnte noch Ablaufdiagramm eingefügt werden.}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{img/ausschnittModullisteHsluInformatik.PNG}
...
%https://www.youtube.com/watch?v=sn9HWh-rg9o&ab_channel=MicrosoftAzure
...
\todo{Hinzufügen Diagramm E-Mail Infrastruktur}
...
\todo{Log eintrag Beispiel anzeigen}
...
\todo{dritter Punkt fertig schreiben.}
\begin{enumerate}
